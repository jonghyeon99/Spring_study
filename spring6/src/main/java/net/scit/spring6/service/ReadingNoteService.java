...
public class ReadingNoteService {
	...
	public void insertReading(ReadingNoteDTO readingDTO) {
		...
		if(temp.isPresent()) {
			ReadingNoteEntity entity = ReadingNoteEntity.toEntity(readingDTO);	
			entity.setBookEntity(temp.get());
	 ...
	 * @return
	 */
	public List<ReadingNoteDTO> selectAll() {
		List<ReadingNoteEntity> temp = readingRepository.findAll();
		List<ReadingNoteDTO> list = new ArrayList<>(); 
		
		temp.forEach((entity) -> list.add(ReadingNoteDTO.toDTO(entity)));
		return list;
	}
}
